...
def load_station_data() -> pd.DataFrame:
    ...
    for station in root.findall(".//Station"):
        ...
        if (
            ...
            and not (-10.0 <= temperature <= 50.0)
        ):
            temperature = None

        if (
            tmax is not None
            and not (-10.0 <= tmax <= 50.0)
        ):
            tmax = None

        if (
            tmin is not None
            and not (-10.0 <= tmin <= 50.0)
        ):
            tmin = None
...
def create_weather_overlays():
    """
    สร้างภาพ interpolation 4 Layer:
    Tmin, Tmax, Temperature และ Rainfall
    """
    ...
    for field, item in settings.items():
        ...
        station_lon = valid_data["longitude"].to_numpy()
        station_lat = valid_data["latitude"].to_numpy()
        station_values = valid_data[field].to_numpy()
        ...
        if field == "rainfall":
            minimum_value = 0.0
            maximum_value = float(
                np.ceil(station_values.max())
            )
        else:
            minimum_value = float(
                np.floor(station_values.min())
            )
            maximum_value = float(
                np.ceil(station_values.max())
            )
        ...
        axis.set_xlim(minimum_lon, maximum_lon)
        axis.set_ylim(minimum_lat, maximum_lat)
        axis.axis("off")
    ...
    return overlays, station_data, thailand
...
def show_map() -> HTMLResponse:
    ...
    try:
        ...
        station_records = (
            station_data[station_record_columns]
            .where(
                pd.notna(
                    station_data[station_record_columns]
                ),
        ...
        latest_observation_time = (
            format_observation_datetime(
                valid_observation_times.max()
            )
            if not valid_observation_times.empty
            else "Unknown"
        )
        ...
        <div class="map-title">
            ...
            <div class="map-observation-time">
                Observation time:
                {html.escape(latest_observation_time)}
            </div>
        </div>
        """
        ...
        (function() {{
            ...
            function updateLegend() {{
                const metadata = getCurrentLayer();
